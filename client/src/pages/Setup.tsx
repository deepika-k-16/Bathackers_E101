    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 font-sans">
      <div className="w-full max-w-2xl">
        <div className="text-center mb-10">
          <h1 className="text-3xl font-bold text-slate-900 font-display">Let's build your profile</h1>
          <p className="text-slate-500 mt-2">We need to understand your constraints to build your engine.</p>
        </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                {errors.businessCategory && <span className="text-red-500 text-xs">{errors.businessCategory.message}</span>}
              </div>

              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Growth Goal</label>
                <select
                  {...register("growthGoal")}
                  className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                >
                ...
                <div className="grid grid-cols-3 gap-2">
                  {["local", "young", "general"].map((opt) => (
                    <label key={opt} className="cursor-pointer">
                      <input type="radio" value={opt} {...register("targetAudience")} className="peer sr-only" />
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700">Budget Constraint</label>
                <div className="grid grid-cols-3 gap-2">
                  {["zero", "low", "medium"].map((opt) => (
                    <label key={opt} className="cursor-pointer">
                      <input type="radio" value={opt} {...register("budget")} className="peer sr-only" />
                      <div className="text-center py-2 rounded-lg border border-slate-200 text-sm font-medium text-slate-600 peer-checked:bg-primary/10 peer-checked:border-primary peer-checked:text-primary transition-all capitalize">
                        {opt}
                      </div>
                    </label>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {["< 5 hrs", "5-10 hrs", "> 10 hrs"].map((opt) => (
                    <label key={opt} className="cursor-pointer">
                      <input type="radio" value={opt} {...register("timeAvailability")} className="peer sr-only" />
